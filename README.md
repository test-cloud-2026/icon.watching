...
> **これは「自動化ツール」です。** 本来なら人が画面を見ながら手作業で数えるしかなかった「リアクションの種類別集計」を、`MutationObserver` による画面監視で自動的に検出・重複排除・集計します。人が行うのは表示のオン/オフだけで、検出からカウントまではすべて自動です。Web アプリ(公開 URL でアクセスするもの)ではなく、ブラウザ上で動く自動化ツールという位置づけです。

![カウント中のデモ](docs/demo-live.png)

## 実行例

上の画像は、実際の Google Meet 会議で 😮23件・😢5件・🤔34件とリアクションを種類別に数えている様子です(名前・会議コードはぼかしています)。

以下は開発初期(配色変更前)の録画・GIFです。観察員の見た目は現在と異なりますが、パネルの表示位置や動作の流れは同じです。
...
▶ **動画で見る**: [docs/demo.mp4](docs/demo.mp4)(クリックすると GitHub 上で再生できます。開発初期の配色時点の録画です)
